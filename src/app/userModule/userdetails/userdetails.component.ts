...
export class UserdetailsComponent implements OnInit {
  ...
  ELEMENT_DATA =  [ ];
    ...
    columnsToDisplay2 = ['name','moduleName', 'entityName', 'weight', 'symbol', 'position'];
    columnsToDisplay3 = ['name', 'moduleName', 'entityName', 'weight', 'symbol', 'position'];
...
maxdateForList:any;
  ...
  openDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if(result != undefined && result.defaultBranch != undefined && this.checkInPriviArryP(this.testt,result.defaultBranch[0])==true){
      ...
      if(result != undefined && result.defaultBranch != undefined && this.checkInPriviArryP(this.testt,result.defaultBranch[0])==false){
  ...
  openDialogPrivi() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if(result != undefined && result.SearchTypeDefault != undefined){
  ...
  priviarr.saveRevertButton = 'Save dates';
   ...
   let d3 = new Date(effectiveDate);
      ...
      this.effectiveDateFlag = false;
         ...
         priviarr['effectiveDateFlag']= false;
         }
       ...
       if((d1.getTime() >= d2.getTime()) || (d1.getTime() >= d3.getTime())){
         ...
         priviarr.saveRevertButton = 'Save dates';
          priviarr.effectiveDateFlag= true;
          priviarr.expiryDateFlag= true;
         }
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      const headers = new HttpHeaders({correlationId: "1",branchCode: "1",journeyId: "1",userId: userId,
        originUserType: "1"});
      this.httpservice
        .get(AppSetting.API_ENDPOINT + "secure/v1/users/" + userId, {headers: headers})
        .subscribe(data => {
          let success: any = {};
          success = data;
  ...
  validateRoleAtleastOne(){
    ...
    if( this.roledata.length == this.postRoleData.length ){
      ...
      this.roledata.filter(obj => {
        if(obj.addOrRemoveOrUpdate  == 'add' || obj.addOrRemoveOrUpdate  == 'UPDATE'){
          this.roleSubmitDis = false;
        }
      });
...
//  validate for atleast one Role is checked
  ...
  addOrRemoveOrUpdate = "ADD";
    ...
    this.httpservice
      .put<any>(AppSetting.API_ENDPOINT + "secure/v1/users", this.editdata, {
        headers: headers
      })
      .subscribe(data => {
...
emptyhandly(whichItem){
  ...
  priviEffect(whichItem, data) {
    ...
    this.effectiveDateFlag= true;
    // End From Pradeep
     if(whichItem.effectiveDate <(moment( this.todayDt).format('YYYY-MM-DD'))){
          whichItem.expiryDate_min = this.todayDt;
      }
     else{
          whichItem.expiryDate_min = moment(whichItem.effectiveDate).add(1, 'days').toDate();
      }
    ...
    editRoleEffectiveDate(whichItem, data) {
      ...
      var roleDataIndex = this.postRoleData.findIndex(object=>(object.editDate));
      if(roleDataIndex >=0){
             var roleObject = this.postRoleData[roleDataIndex];
             roleObject.editDate=false;
             }
       var roleIndex = this.roledata.findIndex(object=>(object.editDate));
             if(roleIndex >=0){
                    var roleObj = this.roledata[roleIndex];
                    roleObj.editDate=false;
                    }


      var saveIcon= this.postRoleData.filter(o=>o.saveIcon);
            if(saveIcon.length<=0){
            this.roleExpiryDateFlag = true;
            this.roleEffectiveDateFlag = true;
            }
            else{
            this.roleExpiryDateFlag = false;
            this.effectiveDateFlag = false;
            }
     ...
     var originalPrivilegeBranchData = this.clonedPrivilegeBranchData.findIndex(object=>(object.branchId === whichItem.branchId));
        ...
        for (let i = 0; i < this.testt.length; i++) {
              ...
              if (element.branchId == whichItem.branchId) {
                ...
                if(element.addOrRemoveOrUpdate != 'ADD' && element.addOrRemoveOrUpdate != 'Remove'){
...
roleSrchData=[];
  ...
  RoleData() {
    ...
    this.httpservice
      .get<any>(AppSetting.API_ENDPOINT + "secure/v1/roles", { headers: headers })
      .subscribe(data => {
        let ob = ErrorConstants.validateException(data);
        if(ob.isSuccess){
  ...
  role() {
    ...
    this.httpService
      .get<any>(
        AppSetting.API_ENDPOINT + "secure/v1/users/" + userId + "/roles",
        { headers: headers }
      )
      .subscribe(data => {
        let ob = ErrorConstants.validateException(data);
        if(ob.isSuccess){
          ...
          let currentDate;
          if(this.todayDt == undefined){
          currentDate = moment( new Date()).format('YYYY-MM-DD');
          }
          else{
          currentDate = moment( this.todayDt).format('YYYY-MM-DD');
          }
          this.roledata.map(v=>{
            v.checked=true;
            v.effectiveDate_min = currentDate;
            if(v.effectiveDate > currentDate){
            v.expiryDate_min = moment(currentDate).add(1, 'days').toDate();
            }
            else{
            v.expiryDate_min = currentDate;
            }
          })
...
saveRoleChange(whichItem, vall, j) {
    ...
    if(whichItem.expiryDate != undefined){
    whichItem.expiryDate =  moment( whichItem.expiryDate).format('YYYY-MM-DD');}
...
var saveIcon= this.postRoleData.filter(o=>o.saveIcon);
  ...
  DefaultData() {
    ...
    this.httpservice
      .get(AppSetting.API_ENDPOINT + "secure/v1/branches/", {
        headers: headers
      })
      .subscribe(data => {
        var dataa = data;
...
function uniqueObj(data){
  ...
  o.permission={
        ...
        if (!((o.subEntityName + o.objectId) in p)) {
            o.permission=[]; p[o.subEntityName + o.objectId] = r.push(o) - 1;
...
export class popforDefaultBranchDetails {
  ...
  advanceDefaultBranchArea() {
   ...
   let apiEndPoint ='';
       ...
       apiEndPoint = AppSetting.API_ENDPOINT +`secure/v1/branches/types`
       }
       else if(this.model.search != "PINCODE"){
       apiEndPoint = AppSetting.API_ENDPOINT + `secure/v1/branches/branchtype/${this.model.search}`;
       }
         var headers = new HttpHeaders({
           branchCode: "B1",
           journeyId: "A1",
  ...
  advanceDefaultBranchAreaById(data) {
        ...
        this.httpservice
          .get<any>(
           AppSetting.API_ENDPOINT + `secure/v1/branches/${this.model.search}/` + this.model.searchByArea.branchId,
            { headers: headers }
          )
          .subscribe(data => {
          let ob = ErrorConstants.validateException(data);
 ...
 var headers = new HttpHeaders({
    ...
    this.httpservice
      .get<any>(
        AppSetting.API_ENDPOINT + `secure/v1/branches/branchtype/` + this.model.searchByType,
        { headers: headers }
      )
      .subscribe(data => {
      let ob = ErrorConstants.validateException(data);
...
export class popforPriviBranchDetails  {
 ...
 selectAll($event) {
  if($event.checked==true){
  if(this.tableData.responseData){
    this.tableData.responseData.forEach(element => {
