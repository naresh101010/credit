...
RUN mkdir /usr/local/apache2/htdocs/associate_cntr_cargo_ui
COPY httpd.config /usr/local/apache2/conf/httpd.conf
COPY ./dist/ /usr/local/apache2/htdocs/associate_cntr_cargo_ui/ 
RUN chmod -R 755 /usr/local/apache2/htdocs/associate_cntr_cargo_ui/
EXPOSE 80