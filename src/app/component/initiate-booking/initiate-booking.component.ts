...
import { WaybillEWaybillComponent } from 'src/app/dialog/waybill-ewaybill/waybill-ewaybill.component';
...
@Component({
  selector: 'app-initiate-booking',
  templateUrl: './initiate-booking.component.html',
  styleUrls: ['./initiate-booking.component.css']
})
export class InitiateBookingComponent implements OnInit {
  ...
  getConsigneeDetailByid(id) {
    this.$bookingInfo.getConsignorConsigneeById(id).subscribe(
      (resp) => {
        // console.log('resp getConsigneeDetailByid getConsigneeDetailByid getConsigneeDetailByid', resp)
  ...
  validateWaybillNummanuallyFirst(event) {
  ...
  saveWayBill(type) {
    ...
    if (this.bookingInfoObj.autoWaybill !== true) {
      if (this.bookingInfoObj.waybillNumber === '' || !this.isValidManual) {
    ...
    if (this.bookingInfoObj.packageList) {
      for (let pkg of this.bookingInfoObj.packageList) {
        for (let sel of this.selectedProductCategoryCardList) {
          pkg.productCategoryId = sel.id
        }
      }
    }
    ...
    // console.log('this.bookingInfoObj.packageList',this.bookingInfoObj.packageList);
    // console.log(isPresent, selectedProductStr);
    ...
    // console.log('this.packagelist this.bookingInfoObj', this.bookingInfoObj);
    if (this.bookingInfoObj.packageList && this.bookingInfoObj.packageList.length) {
      ...
      // console.log('this.bookingInfoObj.actualweight', this.bookingInfoObj.actualWeight);
      // console.log('this.bookingInfoObj totalWeight', totalWeight);
    ...
    //   })
    //   console.log('this.bookingInfoObj.totalPackageCount', this.bookingInfoObj.totalPackageCount, totalpackage)
    //   if (parseInt(this.bookingInfoObj.totalPackageCount) != totalpackage) {
    //     this.appComp.showMessage(`Total packages and No. of packages are not matching !`, 'danger')
    //     return;
    //   }
  ...
  gettingWaybillId() {
    // console.log('this,bookinginfo', this.bookingInfoObj);
    // return;
    this.spinner.show();
    ...
    if (!this.bookingInfoObj.waybillId || this.bookingInfoObj.waybillId === 'undefined') {
      delete this.bookingInfoObj.waybillId
    }
    const id = sessionStorage.getItem("bookingReqId");
    if (id) {
      this.bookingInfoObj.bookingReqId = Number(id);
    }
    console.log('booking info obj', this.bookingInfoObj);
    // return;
  ...
  RefreshOnChange(type = null) {
    ...
    if (this.fieldsetDisabled == false) {
      ...
      if (type == 2 || type == 1) {
        // console.log('herrrrrrrrr refresh')
  ...
  rCList = []
  getRateCardResponse(type) {
    this.$bookingInfo
      .gettingRatecardByContract(this.bookingInfoObj.sfxPrcContractcode, type, this.headerData2)
      .subscribe((response) => {
        if (response) {
          ...
          if (this.rCList.length == 1) {
            this.getCommercialByRateCard(this.rCList[0].id);
          }
        }
      })
  }
  getRateCardBySafexId() {
    ...
    this.$bookingInfo
      ...
      .subscribe((response) => {
        console.log('getRateCardBySafexId', response);
        ...
        if (response) {
          ...
          console.log('this.isCreditCheck ', this.isCreditCheck);
          // if (this.isCreditCheck == false) {
          ...
          //   }
          // }
          console.log('rate card list', TotalRateCard);
          if (this.bookingInfoObj.serviceOfferingId && this.bookingInfoObj.waybillId) {
            ...
            let abc = [];
            var unique = [];
            console.log('inside else condition', TotalRateCard);
            ...
            let isPresent = [];
            console.log('this.serviceOfferingReferenceList', this.serviceOfferingReferenceList);
          ...
          console.log('this.rate card list', this.RateCardList);
          if (this.sfxObj.msaCustId) {
            this.bookingInfoObj.msaCustId = this.sfxObj.msaCustId;
          }
          if (this.bookingInfoObj.rateCardId) {
  ...
  searchSafexId(sfxId) {
    ...
    if (this.selectedValue == "SFX" || this.selectedValue == "PRC") {
      ...
      if (strlength > 2 && sfxId) {
        ...
        this.sfxCode = sfxId;
        // console.log('before getiing rate card lets check', this.contractType);
        this.$bookingInfo.getSfxCode(sfxId, this.contractType, this.headerData).subscribe(
          (response) => {
            // console.log('before getiing rate card lets check', response);
  ...
  getServiceOffering() {
    ...
    this.$bookingInfo.getAllServiceOffering(this.headerData).subscribe((response) => {
      ...
      if (response) {
        ...
        this.serviceOfferingListCopy = response;
  ...
  getCommercialByRateCard(ratecard) {
    // console.log('getCommercialByRateCard getCommercialByRateCard')
    ...
    try {
      if (this.fieldsetDisabled == false) {
        ...
        this.$bookingInfo
          ...
          .subscribe((Response) => {
            // console.log('getCommercialByRateCard', Response)
            this.spinner.hide();
            if (Response) {
            ...
            // console.log('commercialTypeListcommercialTypeList', this.commercialTypeList)
  ...
  checkTimeWithCutOffTime() {
    console.log('this.branchiddddddddddddddd', this.branchId)
    const selectedBranch = sessionStorage.getItem('branchId');
    console.log('this.selectedBranch', selectedBranch)
  ...
  checkBillableWeightFlag() {
    // console.log('checkBillableWeightFlag checkBillableWeightFlag checkBillableWeightFlag checkBillableWeightFlag')
    this.$bookingInfo.billableWeightFlag(this.contractType, this.bookingInfoObj.rateCardId, this.headerData2).subscribe((response) => {
      if (response) {
        // console.log('checkBillableWeightFlag checkBillableWeightFlag checkBillableWeightFlag checkBillableWeightFlag', response)
  ...
  openEwaybillModal(infoType) {
    ...
    var dialog = this.dialog.open(WaybillEWaybillComponent, {
      width: width,
      panelClass: "bookingDialog",
      data: data,
    });
  ...
  addProductRowByDefault() {
    ...
    if (presentVal == 0) {
      ...
      this.bookingInfoObj.packageList.splice(0, 0, {
        productCategoryId: this.categoryListWithAlisList[0] ? this.categoryListWithAlisList[0].id : null,
  ...
  getFilterProductCategory(product, i) {
    ...
    console.log('this.package detail;s', this.bookingInfoObj.packageList);
  ...
  deletePackageInfoRow(index) {
    // console.log('delete package row');
    // if (this.fieldsetDisabled == false) {
      ...
      this.$bookingInfo
        ...
        .subscribe((response) => {
          ...
          if (response) {
            ...
            // const d = new Date();
            // this.bookingInfoObj.pickUpDate = this.dateUtil.getFormatedDate('DD/MM/YYYY', d)
            if (this.bookingInfoObj.consigneeId) {
            ...
            if (this.bookingInfoObj.invoiceList && this.bookingInfoObj.invoiceList.length) {
              // console.log('inside invoice details')
              this.bookingInfoObj.ncvFlag = 0;
              this.bookingInfoObj.ewaybillAvailableFlag = 1;
            }
            this.localWaybillList = this.bookingInfoObj.invoiceList;
          ...
          // console.log('this.bookinginfo', this.bookingInfoObj.packageList);
        });
    } catch (error) {
      this.spinner.hide();
    }
  ...
  bookingIdDetailsByWaybillApi(id) {
    ...
    this.openBookingService.getBookingRequestDetails(id).subscribe((res: any) => {
      ...
      if (res.data) {
        ...
        if (this.bookingInfoObj.invoiceList && this.bookingInfoObj.invoiceList.length) {
          // console.log('inside invoice details')
          if (this.bookingInfoObj.invoiceList[0].ewaybillNumber && this.bookingInfoObj.invoiceList[0].ewaybillNumber !== '') {
            this.bookingInfoObj.ewaybillAvailableFlag = 1;
          }
        }
        const d = new Date();
        this.bookingInfoObj.pickupDate = this.dateUtil.getFormatedDate('yyyy-MM-DD', d);
        console.log('this.bookingInfoObj.pickupDate', this.bookingInfoObj.pickupDate)
        // this.localBookingObj.pickupDate = this.dateUtil.getFormatedDate('DD/MM/YYYY', d);
        // console.log('this.bookingInfoObj.pickUpDate', this.bookingInfoObj.pickupDate);
        // console.log('this.localBookingObj.pickUpDate', this.localBookingObj.pickupDate);

        // this.bookingInfoObj.packageList = [];
        // if (this.bookingInfoObj.waybillNumber) {
        //   this.validateWaybillNummanuallyFirst(true);
        // }
        if (this.bookingInfoObj.pickupTime) {
          this.bookingInfoObj.pickupTime = this.bookingInfoObj.pickupTime.split(' ')[0];
        } else if (this.bookingInfoObj.pickUpToTime) {

          var time = this.bookingInfoObj.pickUpToTime;
          ...
          this.bookingInfoObj.pickupTime = val;
          // this.sampleBody.pickupTime = val;

          // this.bookingInfoObj.pickupTime = this.bookingInfoObj.pickUpToTime.split(' ')[0];
        }
        this.bookingInfoObj.ncvFlag = 0;
        ...
        // console.log('categoryListWithAlisList categoryListWithAlisList', this.categoryListWithAlisList);
        
        this.checkTimeWithCutOffTime();

        // console.log('this.commercialTypeList.length',this.commercialTypeList.length);
        // console.log('this.bookingInfoObj.serviceOfferingId',this.bookingInfoObj.serviceOfferingId); 
        // console.log('this.invalidPincode',this.invalidPincode)
        // console.log('this.bookingInfoObj.destPincodeId',this.bookingInfoObj.destPincodeId)
        // console.log('this.bookingInfoObj.sfxPrcContractcode',this.bookingInfoObj.sfxPrcContractcode)
        // console.log('this.destPincode',this.destPincode)
        // console.log('this.bookingInfoObj this.bookingInfoObj', this.bookingInfoObj)
        // console.log('this.productcat this.product', this.isProductIndex)
