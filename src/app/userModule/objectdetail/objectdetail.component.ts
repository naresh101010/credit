...
getObjDtl() {
  ...
  this.objectdetailservice.getObjectDetailById(objectId).subscribe((userDtl: any) => {
    ...
    this.objectData = userDtl.data.responseData;
          ...
          if(data.channelId ==33 && data.objectChannelMapStatus==1){
                    this.isWeb = true;}
                    if(data.channelId ==34 && data.objectChannelMapStatus==1){
                    this.isMobile = true;
                    }
          }
...
isChannelUpdate(event, channelId, objectId){
 let object = {
        "isAddOrRemoveOrUpdate": '',
        "channelId": '',
        "status" : 1
    }
if(this.ddata['objectChannelMapDTOS'] == undefined){
this.ddata['objectChannelMapDTOS'] =[];
}
let objectDetails = Object.assign([], this.objectData);
if(this.ddata.objectChannelMapDTOS!= undefined){
let existingObjectIndex = this.ddata.objectChannelMapDTOS.findIndex(object=>(object.objectId === objectId &&
object.channelId == channelId))
if(existingObjectIndex >=0){
this.ddata.objectChannelMapDTOS.splice(existingObjectIndex,1);
}
}

var objectIndex = objectDetails.findIndex(object=>(object.objectId === objectId && object.channelId == channelId));
                object['isAddOrRemoveOrUpdate']= 'UPDATE';
                object.channelId = channelId
                ...
                object.status = 1;
                if(objectIndex<0){
                 object['isAddOrRemoveOrUpdate']= 'ADD';
                     }
                }
                this.ddata.objectChannelMapDTOS.push(Object.assign({}, object));
}
