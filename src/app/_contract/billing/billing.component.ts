...
export class BillingComponent implements OnInit {
  ...
  constructor(private contractservice: ContractService, private dialog: MatDialog, private spinner: NgxSpinnerService, private datePipe: DatePipe,
  ...
  getOpportunityDetails() {
    ...
    this.contractservice.getOportunity(AppSetting.oprtunityId,this.editflow).subscribe(
      opprResult => {
      ...
      if (ob.isSuccess) {
        ...
        result.referenceData.businessTypeList.forEach(businessTypeElement => {
          if (businessTypeElement.id == result.responseData.contract.lkpBizTypeId) {
  ...
  getBillingData() {
      ...
      .subscribe(billingData => {
        ...
        if (data.responseData && data.responseData.length > 0) {
          this.billingLevelList.forEach(billingLevelelement => {
            if (billingLevelelement.id === data.responseData[0].lkpBillingLevelId) {
        ...
        if(this.billingLevel == 'MSA') {
          ...
          this.rateCardList = [];
          offeringId = 0;
          this.contractservice.getRateCardDetail(AppSetting.contractId,this.editflow).subscribe(
            result => {
        ...
        if (data.referenceData.billingByList) {
          if (this.billingLevel != '' && this.billingLevel == 'MSA' && this.businessType != "ANYWHERE TO ANYWHERE") {
            data.referenceData.billingByList.forEach(billingByElement => {
              if ((billingByElement.lookupVal == 'CONSOLIDATION' || billingByElement.lookupVal == 'DESTINATION BRANCH WISE' || billingByElement.lookupVal == 'DESTINATION STATE WISE'))
        ...
        if (data.referenceData.billingSubLevelList) {
          ...
          this.billingSubLevelList.forEach(billingSubelement => {
            if (billingSubelement.id == offeringId && billingSubelement.lookupVal != 'ALL') {
        ...
        if(this.billingData && this.billingData.billingBy) {
          this.billingByList.forEach(billingByElement => {
            if (billingByElement.id == this.billingData.billingBy[0].billingByLevelMapId) {
        ...
        && !(this.businessType === 'OUTBOUND' && this.billingByLevelName==='DESTINATION BRANCH WISE')) {
          this.contractservice.getAssignBranchDetailByCntr(AppSetting.contractId, this.editflow, this.billingLevel).subscribe(
            result => {
              let ob = ErrorConstants.validateException(result);
              if (ob.isSuccess) {
                let branchData = JSON.parse(JSON.stringify(result.data.responseData));
                ...
                branchData.forEach(branchElement => {
                  ...
                  if (!exist) {
                    if (this.billingByLevelName === 'BOOKING BRANCH') {
                      newBillingBy.billingBranchId = branchElement.bkngBranchId;
                      newBillingBy.billingBranchName = branchElement.bkngBranchName;
                      newBillingBy.assignBranchName = branchElement.bkngBranchName;
                    } else {
                      newBillingBy.assignBranchId = branchElement.bkngBranchId;
                      newBillingBy.assignBranchName = branchElement.bkngBranchName;
                    }
                    branchToBillMap.set(newBillingBy.assignBranchName, newBillingBy);
                  }
                });

                for (let index = 0; index < this.billingData.billingBy.length; index++) {
                ...
                if (billToBranchMap.size > 0 && branchToBillMap.size > 0) {
                  ...
                  for (let j = 0; j < billingIndexList.length; j++) {
                    this.billingData.billingBy[billingIndexList[j]].assignBranchId = brnchBillData[j].assignBranchId;
                    this.billingData.billingBy[billingIndexList[j]].billingBranchId = brnchBillData[j].billingBranchId;
                    this.billingData.billingBy[billingIndexList[j]].assignBranchName = brnchBillData[j].assignBranchName;
                    this.billingData.billingBy[billingIndexList[j]].billingBranchName = brnchBillData[j].billingBranchName;
                  }

                  if (billingIndexList.length !== brnchBillData.length) {
                    for (let i = billingIndexList.length; i < brnchBillData.length; i++) {
                      this.billingData.billingBy = this.billingData.billingBy.concat(brnchBillData[i]);
                    }
                  }
    ...
    this.spinner.show();
    this.contractservice.getBillingByMSA(AppSetting.msaCustId)
      .subscribe(billingData => {
    ...
    this.contractservice.getCneeCnorData(AppSetting.msaCustId)
      .subscribe(cneeCnorData => {
        ...
        if (this.billingData && this.billingData.billingCneeCnorMap && this.billingData.billingCneeCnorMap.length > 0) {
          this.billingData.billingCneeCnorMap.forEach(billingCneeCnorElement => {
            if (this.cneeList && this.cneeList.length > 0) {
            for (var i = 0; i < this.cneeList.length; i++) {
            ...
            if (result && result.element == 'CONSIGNEE') {
              result.cneeList.forEach(cneeElement => {
                if (cneeElement.ischecked == true) {
                  newBillingCneeCnorMap.push({ cneeId: cneeElement.id, cnorId: 0, id: null,cneeName: cneeElement.name });
                }
              });
              if (this.billingData.billingCneeCnorMap && this.billingData.billingCneeCnorMap.length > 0) {
                ...
                var id = null;
                 ...
                 cneeCnorMap.push({ cneeId: elementNew.cneeId, cnorId: 0, id: id, cneeName: elementNew.cneeName });
                });
                this.billingData.billingCneeCnorMap = JSON.parse(JSON.stringify(cneeCnorMap));
              } else {
                this.billingData.billingCneeCnorMap = JSON.parse(JSON.stringify(newBillingCneeCnorMap));
              }
                ...
                if (cnorElement.ischecked) {
                  result.cneeList.forEach(element1 => {
                    if (element1.ischecked) 
                      newBillingCneeCnorMap.push({ id: null, cneeId: element1.id, cnorId: cnorElement.id, cneeName: element1.name, cnorName: cnorElement.name });
                  });
                }
              });
              ...
              if (this.billingData.billingCneeCnorMap && this.billingData.billingCneeCnorMap.length > 0) {
                ...
                var id = null;
                  this.billingData.billingCneeCnorMap.forEach(billingCneeCnorElement => {
                    if (elementNew.cneeId == billingCneeCnorElement.cneeId && elementNew.cnorId == billingCneeCnorElement.cnorId) {
                    id = billingCneeCnorElement.id;
                    }
                  });
                  cneeCnorMap.push({ cneeId: elementNew.cneeId, cnorId: elementNew.cnorId, id: id, cneeName: elementNew.cneeName,cnorName:elementNew.cnorName });
                });
                this.billingData.billingCneeCnorMap = JSON.parse(JSON.stringify(cneeCnorMap));
              } else {
                this.billingData.billingCneeCnorMap = JSON.parse(JSON.stringify(newBillingCneeCnorMap));
    ...
    this.contractservice.getAllStates().subscribe(result => {
      ...
      if (ob.isSuccess) {
        result.data.responseData.forEach(stateElement => {
          this.stateList.push({ id: stateElement.id, lookupVal: stateElement.stateName,descr:stateElement.stateName })
        });
  ...
  postBillingData(nextKey) {
    ...
    if (this.selectOffering) {
      ...
      if (this.billingData.billingOfferingMap && this.billingData.billingOfferingMap.length > 0 && this.billingData.billingOfferingMap[0].serviceOfferingId > 0) {
        ...
        this.billingOfferingList.forEach(billingOfferingElement => {
          this.billingData.billingOfferingMap.push({id:null,serviceOfferingId:billingOfferingElement.serviceOfferingId, status: null});
        });
      }
    }
    ...
    this.contractservice.postBillingData(this.billingData,this.editflow).subscribe(result => {
      ...
      if (ob.isSuccess) {
        ...
        if (nextKey == 1) {
          if (this.billingLevel === 'RATE CARD') {
            this.contractservice.validateBilling(AppSetting.contractId, this.editflow).subscribe(validateResult => {
              let validationMessages = '';
              if (validateResult.data.responseData) {
                validationMessages = JSON.stringify(validateResult.data.responseData);
  ...
  loadRateCard(rateCardId) {
    ...
    if (this.allBillingData ) {
      this.allBillingData.forEach(allBillingElement => {
        if (allBillingElement.entityId == rateCardId && allBillingElement.lkpBillingLevelId == this.billingData.lkpBillingLevelId){
      ...
      this.billingByList.forEach(billingByElement => {
        ...
        if (this.billingData && this.billingData.billingCneeCnorMap) {
          this.contractservice.getCneeCnorData(AppSetting.msaCustId).subscribe(cneeCnorData => {
                 let ob = ErrorConstants.validateException(cneeCnorData);
            if (ob.isSuccess) {
                  for (let k = 0; k < this.billingData.billingCneeCnorMap.length; k++) {
                    this.cneeList = cneeCnorData.data.referenceData.msaCneeList;
                    this.cnorList = cneeCnorData.data.referenceData.msaCnorList;
                    if (this.cneeList && this.cneeList.length > 0) {
...
export class BranchDialogBox {
  ...
  constructor(private contractservice: ContractService, 
    ...
    public dialogRef: MatDialogRef<BranchDialogBox>,  private tosterservice: ToastrService,
