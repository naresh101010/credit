...
@Component({
  selector: 'app-create-wayill-api',
  templateUrl: './create-waybill-api.component.html',
  styleUrls: ['./create-waybill-api.component.scss']
})
export class CreateInitiateWaybillBookingComponent implements OnInit {
 ...
 @ViewChild("step1", null) consigneeInfo: any;
  ...
  consignorId = this.bookingInfoObj.consignorId;
    this.$bookingInfo.getConsignorConsigneeById(consignorId).subscribe(
      (resp) => {
        // console.log('resp', resp)
...
this.appComp.showMessage(`Please Select RateCard !`, 'danger');
  ...
  validatePackaging(){
    ...
    //   this.bookingInfoObj.packageList.forEach(element => element.commercialId = this.bookingInfoObj.commercialId);
    //  }
   }
  }
      ...
      .subscribe((Response) => {
        if (Response) {
          if(this.bookingInfoObj.ewaybillAvailableFlag){
            ...
            this.bookingInfoObj.invoiceList.forEach((element , index) => {
              if(element.ewaybillInvoiceDetail){
               element.ewaybillNumber = element.ewaybillInvoiceDetail.ewaybillNumber
              }
            });
          }
...
saveConsignorConsigneeInfo(){
  ...
  saveWayBill(type) {
    ...
    // else{
    //   var localList  = [] as any;
    //    localList = this.bookingInfoObj.packageList.filter(elem=> elem.commercialId == this.bookingInfoObj.commercialId)
    // }
    if (this.bookingInfoObj.packageList && this.bookingInfoObj.packageList.length) {
      this.bookingInfoObj.packageList.forEach((elm , index) => {
       let seletedindex  = this.selectedProductCategoryCardList.findIndex(elem=> elem.id == elm.productCategoryId)
    ...
    this.$bookingInfo.getLoad(this.branchId).subscribe((Response) => {
      ...
      if (sessionStorage.getItem("bookingReqId")) {
        // console.log('this.getwaybibookingReqIdlldetails')
          ...
          this.bookingInfoObj.waybillId = waybillId;
          if (waybillId) {
            ...
            if (this.bookingInfoObj.invoiceList) {
              this.bookingInfoObj.invoiceList.forEach((element) => {
                element.invoiceNumber =
                  ...
                  element.ewaybillNumber =
                  element.ewaybillInvoiceDetail.ewaybillNumber;
        ...
        if (this.RateCardList.length == 1 && this.commercialTypeList.length == 1) {
          setTimeout(() => {
           ...
           if(this.consigneeInfo.valid){
             ...
             this.getConsignorDetailByid(this.consignorId);
            this.isExpand = false;
            this.consignerisExpand = true;
           }
          }, 2000);
          ...
          .subscribe((response) => {
            ...
            if (response) {
              ...
              if (response.prdctCtgy.trim() == "GENERAL") {
                this.isPeaceType = false;
                this.categoryListWithAlisList = [...this.tempcategoryList];
                if (this.sampleBody.packageList != undefined) {
                  if(this.sampleBody.packageList.length){
                    let UniqueList = [];
                    if (this.sampleBody.packageList.length) {
                      this.bookingInfoObj.packageList = [...this.sampleBody.packageList];
                      UniqueList = this.sampleBody.packageList.filter((thing, index, self) =>
              ...
              //       elem.commercialId = this.bookingInfoObj.commercialId;
              //   });
              // }
              } else {
                this.categoryListWithAlisList = [...[response]];
                if (this.categoryListWithAlisList.length == 1) {
                  ...
                  if(this.sampleBody.packageList.length){
                    this.sampleBody.packageList.forEach(elem => {
                      if(elem.productCategoryId ==this.selectedProductCategoryList){
                        this.bookingInfoObj.packageList.push(elem);
                      }
                    });
                  }
...
getWaybillByReqId(reqId){
  ...
  this.$bookingInfo.getWaybillByRequestId(obj , this.headerData).subscribe((response)=> {
    if(response.responseData.length){
      ...
      this.bookingInfoObj.waybill = response;
     this.editWaybillDetails();
    }
    else{
      ...
      this.$bookingInfo
        ...
        .subscribe((response) => {
          ...
          if (response) {
            ...
            let abc;
            if(this.bookingInfoObj.temppickupTime){
              abc = this.bookingInfoObj.temppickupTime.split(':');
          ...
          if(this.bookingInfoObj.chargeWeight){
            ...
            if(this.bookingInfoObj.packageList == undefined){
              ...
              if(this.preBookingRequestObj.totalWeight){
               ...
               if(this.preBookingRequestObj.consignorId){
                 this.bookingInfoObj.consignorId = this.preBookingRequestObj.consignorId;
              }
                if(this.preBookingRequestObj.consigneeId){
    ...
    try {
      ...
      this.openBookingService.getBookingRequestDetails(id)
        .subscribe((response : any) => {
          ...
          //Priyanka
          if(this.preBookingRequestObj.invoiceList){
             this.bookingInfoObj.invoiceList = this.preBookingRequestObj.invoiceList;
          }
          ...
          if(this.preBookingRequestObj.invoiceList && this.preBookingRequestObj.invoiceList.length &&
          this.preBookingRequestObj.invoiceList[0].ewaybillNumber){
          this.bookingInfoObj.ewaybillAvailableFlag =1;
          this.preBookingRequestObj.ewaybillAvailableFlag =1;
          }
  ...
  getPreBookingResponse(){
        ...
        this.bookingInfoObj.invoiceList.forEach((element , index) => {
          if(element.ewaybillInvoiceDetail){
           element.ewaybillNumber = element.ewaybillInvoiceDetail.ewaybillNumber
            ...
            this.bookingInfoObj.invoiceList[index].ewaybillInvoiceDetail = {};
            this.bookingInfoObj.invoiceList[index].ewaybillInvoiceDetail.ewaybillNumber = element.ewaybillNumber
          }
        });
      }
    ...
    //   }
      ...
      this.localWaybillList = this.bookingInfoObj.invoiceList;
      //Priyanka
      if(this.bookingInfoObj.invoiceList && this.bookingInfoObj.invoiceList.length== 0 && this.preBookingRequestObj.invoiceList){
         this.bookingInfoObj.invoiceList = this.preBookingRequestObj.invoiceList;
      }
      if(this.bookingInfoObj.packageList && this.bookingInfoObj.packageList.length==0 && this.preBookingRequestObj.packageList){
         this.bookingInfoObj.packageList = this.preBookingRequestObj.packageList;
      }
