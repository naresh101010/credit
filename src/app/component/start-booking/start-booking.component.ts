...
import { CancelcompComponent } from 'src/app/dialog/cancelcomp/cancelcomp.component';
...
this.lastDayEndDate = res.data.responseData.dayendDate;
      }  
    }, (err: any) => {
      console.log(err);
    })
  ...
  checkValidatior(){
    // let requiredField = this.consigneeInfo.form.value;
  //   if(!requiredField.branchMOPLookupId || !requiredField.contractOption || !this.bookingInfoObj.destPincodeId || !requiredField.serviceOfferingId  || !requiredField.temppickupTime ||  (this.RateCardList.length && !requiredField.sfxPrcContractcode &&)){
  //     this.appComp.showMessage(`Please Fill Required Field !`, 'danger');
  //   return true;
...
this.appComp.showMessage(`Please Select RateCard !`, 'danger');
  ...
  pickUpTimeInvalid : boolean = false;
  checkpreviousValue(event){
    if(event){
      // if(event == "00:00"){
      //   this.bookingInfoObj.temppickupTime = '';
      //   this.pickUpTimeInvalid = true;
      // }
      if(event.length < 5){
        this.pickUpTimeInvalid = true;
      }
      else{
        this.pickUpTimeInvalid = false;
      }
  ...
  saveBookingInfoDetails(){
    ...
    if(this.consigneeInfo.valid){
      if(!this.bookingInfoObj.prcCntrId){
        this.bookingInfoObj.sfxPrcContractcode = '';
      }
  ...
  gettingWaybillNumberById() {
    ...
    this.$bookingInfo
      ...
      .subscribe((Response) => {
        if (Response) {
          this.bookingInfoObj.waybillNumber = Response.waybillNumber;
          this.bookingInfoObj.wayblCurrStatusLookupId = Response.wayblCurrStatusLookupId;
          this.mopChanged = true;
          ...
          if(Response.packageList){
            this.bookingInfoObj.packageList = [...Response.packageList];
            this.bookingInfoObj.packageList.forEach((elm,index)=>{
          ...
          this.bookingInfoObj.previousbranchMOPLookupId = this.bookingInfoObj.branchMOPLookupId;
          this.IsCrate = false;
          // this.appComp.showMessage(
      ...
      if (type == 2) {
        if (str.length < 10) {
          ...
          if (this.MopName != 'CREDIT') {
            ...
            this.isGeneralRetail = false;
            this.categoryListWithAlisList = [...this.tempProductCategoryWithAlisList];
...
mopChanged : boolean = true;
  mopDialog(type){
      if(this.bookingInfoObj.wayblCurrStatusLookupId){
        var waybillstatusObj = this.allLookupList.find(elem=> elem.id == this.bookingInfoObj.wayblCurrStatusLookupId)
      }
      if(this.mopChanged){
      if(this.bookingInfoObj.waybillId && waybillstatusObj){
       if( ( waybillstatusObj.lookupVal.trim() == 'FREIGHT CALCULATED' || waybillstatusObj.lookupVal.trim() == 'WAYBILL GENERATED') ){
        var dialog = this.dialog.open(MopDialogComponent, {
          width: '50rem',
          disableClose: true,
        ...
        dialog.afterClosed().subscribe((response) => {
          ...
          if (response === true) {
            if(type == 'mop'){
              let toPay = this.mopList.find(elem => elem.branchMOPLookupId == this.bookingInfoObj.previousbranchMOPLookupId)
              this.bookingInfoObj.branchMOPLookupId = toPay.branchMOPLookupId;
              this.selectedMopName = toPay.branchMOPValue;
              this.MopName = toPay.branchMOPValue;
            }
            if(type == 'pincode'){
              if(this.pincodeList.length == 1){
                this.destPincode = this.pincodeList[0].pincode;
                this.bookingInfoObj.destPincodeId = this.pincodeList[0].id;
                }
            }
           return;
          }
          if (response == 1) {
            this.mopChanged = false;
            if(type == 'mop'){
            this.contractOption = 3;
            this.selectedValue = 'MSA';
            this.getSfxonMop(this.bookingInfoObj.branchMOPLookupId);
            this.RefreshOnChange(1);
          }
        ...
        if (response) {
          ...
          if (this.isCreditCheck == false) {
            ...
            if (isPrcRateCard.length) {
              ...
              if(this.selectedMopName.trim() == 'FOC'){
                this.appComp.showMessage(`No Ratecard is mapped with Selected  MOP`, "danger");
              }
              else{
          ...
          if (this.RateCardList.length == 1) {
            this.bookingInfoObj.rateCardId =this.RateCardList[0].id;
            this.getCommercialByRateCard(this.RateCardList[0].id);
          }
          if (this.bookingInfoObj.waybillId && this.bookingInfoObj.rateCardId && this.RateCardList.length > 1) {
        ...
        this.$bookingInfo.getSfxCode(sfxId, this.contractType, this.headerData).subscribe(
          (response) => {
            if (response) {
              ...
              if (isInbound && !this.bookingInfoObj.waybillId && !this.bookingInfoObj.wayblConvertedFlag) {
      ...
      this.$bookingInfo.getWaybillById(this.searchWayBill).subscribe(
        (response) => {
          if (response) {
            ...
            // console.log(this.bookingInfoObj.invoiceList,'this.bookingInfoObj.invoiceList')
  ...
  onClear() {
    ...
    if (this.pincodeList.length > 0) {
      if(this.pincodeList.length == 1 && this.bookingInfoObj.wayblCurrStatusLookupId){
        this.bookingInfoObj.destPincode = this.pincodeList[0].pincode;
      }
      if (this.bookingInfoObj.destPincode) {
        return;
      } else {
        this.pincodeList = [];
        this.hubList = [];
 ...
 checkTimeWithCutOffTime() {
  ...
  tempProductCategoryWithAlisList : Array<any>=[];
  getAlProductcategoryWithAlis() {
    this.$bookingInfo.getProductCategoryByAlis(this.headerData).subscribe((response) => {
      if (response) {
        ...
        this.tempProductCategoryWithAlisList = [... this.categoryListWithAlisList];
  ...
  openAddNewConsignee(lkpConsigntypeId) {
    ...
    if (this.fieldsetDisabled == false) {
      ...
      let data = {
        ...
        msaCustId : this.bookingInfoObj.msaCustId ? this.bookingInfoObj.msaCustId : undefined ,
        type: 'consignee'
      } as any;
      if (this.bookingInfoObj.invoiceList != undefined) {
        ...
        if (response) {
          ...
          this.$bookingInfo.getAllConsignorExistingWithOutRateCard(this.headerData).subscribe((res) => {
            if (res) {
              ...
              if (this.bookingInfoObj.waybillId) {
                let selectedConsignee = this.consignerList.find(elem => this.bookingInfoObj.consignorId == elem.id)
                let findIndex = this.consignerList.findIndex(elem => this.bookingInfoObj.consignorId == elem.id);
               if(findIndex != -1){
                this.consignerList.splice(findIndex, 1);
               }
                if(selectedConsignee){
                  this.consignerList.splice(0, 0, selectedConsignee);
                }
              }
            }
          });
 ...
 if(product.checked == true){
      ...
      this.$bookingInfo
        ...
        .subscribe((response) => {
          ...
          if (response) {
            ...
            this.bookingInfoObj.previousbranchMOPLookupId = response.branchMOPLookupId;
            ...
            this.allLookupList.forEach((elem) => {
              if (elem.lookupVal.trim() == "CREDIT" &&
                this.bookingInfoObj.custTypeLookupId == elem.id && (business_type.lookupVal.trim() == 'OUTBOUND' || this.bookingInfoObj.wayblConvertedFlag)) {
                this.isCreditCheck = true;
                this.contractOption = 1;
                this.selectedValue = 'SFX';
                this.MopName = 'CREDIT';
  ...
  getcommercialRateCardIdByAlias() {
    ...
    if (!this.RateCardList.length && this.bookingInfoObj.consignorId && !this.selectedInboundPackageType && !this.isGeneralRetail && this.selectedProductCategoryCardList && this.bookingInfoObj.serviceOfferingId && !this.isPopUpOpen) {
        ...
        this.$bookingInfo
          ...
          .subscribe((response) => {
            if (response) {
              ...
              if(isGeneralPresent && this.isPopUpOpen){
  ...
  getFreightCharges() {
    this.$bookingInfo.freightCharges(this.bookingInfoObj.waybillId, this.headerData).subscribe(res => {
      if (res) {
        ...
        if (res.totalCharges != undefined || res.totalCharges != null) {
          this.totalCharges = res.totalCharges;
        }
        if (res.grandCharges != undefined || res.grandCharges != null) {
          this.grandCharges = res.grandCharges
        }
        if (res.gstAmount != undefined || res.gstAmount != null) {
...
this.consignerList = [...isPresent];
  ...
  cancelEditWaybill(){
    console.log('here you clicked')
    var dialog = this.dialog.open(CancelcompComponent, {
      width: "50rem",
      panelClass: "bookingDialog",
    });
    dialog.afterClosed().subscribe((res: any) => {
      if (res) {
        this.spinner.show();
        this.waybillService.cancelWaybill(this.bookingInfoObj.waybillId).subscribe(resp => {
          this.spinner.hide();
          this.appComp.showMessage(resp['message'],'success');
          setTimeout(() => {
            this.router.navigate(['/bookings-web-booking/edit-waybill']);
