...
@Component({
  selector: 'app-prebooking-create-waybill',
  templateUrl: './prebooking-create-waybill.component.html',
  styleUrls: ['./prebooking-create-waybill.component.css']
})
export class PrebookingCreateWaybillComponent implements OnInit {
  ...
  //     waybillid: 0,
  //     lkpChannelId: 33,
  //     F: this.bookingInfoObj.bookingRequestTypeId,
  //     bookingRequestId: this.bookingInfoObj.bookingRequestId,
  //     pickupdate: this.bookingInfoObj.pickupDate,
    ...
    this.$bookingInfo.getLoad(this.branchId).subscribe((Response) => {
      if (Response) {
        ...
        this.allLookupList.forEach((element) => {
          ...
          if (element.lookupVal.trim() == 'PRE_BOOKING' && element.lookupTypeVal.trim() == 'BOOKING_REQUEST_TYPE') {
  ...
  addProductCategoryInCardList(selectedProductCategory) {
    ...
    if (Array.isArray(selectedProductCategory)) {
      selectedProductCategory = selectedProductCategory.map((elem, index) => {
        ...
        const el = {};
        const findedProd = this.categoryListWithAlisList.find(el1 => el1.localId == (elem || elem.id))
        console.log('findedProd', findedProd);
        el['prdctCtgy'] = findedProd.prdctCtgy;
        if (index == 0) {
          el['checked'] = true;
          this.isProductIndex = elem.id ? elem.id : elem;
        } else {
          el['checked'] = false;
        }
        return el;
