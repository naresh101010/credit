...
export class CreditdashboardComponent implements OnInit {
  ...
  checkValue;
  ngOnInit() {
    AppSetting.rateCardApplicableFlag=1;
       ...
       if (ob.isSuccess) {
        ...
        let currDateinMilli = new Date(currentDate);
         for (let data of this.msaCust.responseData) {
           ...
           if (data.cntrExpDt && value === 'EXPIRING SOON') {
             ...
             if (!data.cntrExpDt || dateinMilli.getTime() >= currDateinMilli.getTime()) {
  ...
  goToMsa(user) {
    ...
    this.router.navigate(['/prc-contract/msa'], {skipLocationChange : true});
  }
...
//  this.countData(value);
    ...
    if (this.selectedValue === 'PENDING') {
      ...
      this.router.navigate(['/prc-contract/msaoperation'], {skipLocationChange : true});
    } else if (this.selectedValue === 'COMMANDMENT') {
      this.router.navigate(['/prc-contract/msaoperation', {'openDialog': 'true'}], {skipLocationChange : true});
    } else if (this.selectedValue === 'REPORTS') {
      this.router.navigate(['/prc-contract/report', {'openDialog': 'true'}], {skipLocationChange : true});
    }
  }
