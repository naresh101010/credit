 ...
 priviEffect(data){
  for (let i = 0; i < this.newdata.previlegeBranches.length; i++) {
     const element = this.newdata.previlegeBranches[i];
          ...
          let expiryDate=this.convert(this.newdata.previlegeBranches[i].expiryDate);
          if(this.newdata.previlegeBranches[i].expiryDate == null || (expiryDate <= this.newdata.previlegeBranches[i].effectiveDate)){
...
validateUserDetails(){
 this.newdata.userId.length <=0 ?this.isUserIdHasValue =false : this.isUserIdHasValue =true;
      ...
      if(this.newdata.email.length <=0){this.emailPattern =false}
}
openDialog() {
...
checkInPriviArryP = (objArray, val) => {
  ...
  dateChangeEffectiveDate(event, oldValue, expiryDate,j, priviarr){
    ...
    let d1 = moment(new Date(temp),"dd/MM/yyyy").startOf('day') ;
    let d2 = moment(new Date(oldValue),"dd/MM/yyyy") ;
...
createUser(){
  ...
  let ob;
  this.httpservice.post(AppSetting.API_ENDPOINT+'secure/v1/users',this.newdata,).subscribe(
    data => {
...
saveRoleChange(whichItem, vall, j) {
    ...
    if(!whichItem.effectiveDateFlag || whichItem.expiryDateFlag){
...
constructor(@Inject(MAT_DIALOG_DATA) public data: DialogData, private authSer:AuthorizationService, private SpinnerService: NgxSpinnerService, public dialogRef: MatDialogRef<popforDefaultBranch>, private httpservice:HttpClient, public router:Router,private toast: ToastrService,) {}
   ...
   if(obj.length > 2){
    if (this.model.search == "NAME") {
      ...
      let searcgObj=this.model.searchbyname.toUpperCase();
       this.SpinnerService.show();
      this.httpservice.get<any>(AppSetting.API_ENDPOINT + `secure/v1/branches/branchName/${searcgObj}`, { headers: headers }).subscribe(
        data => {
          let ob = ErrorConstants.validateException(data);
          if(ob.isSuccess){
           this.arr =[];
 ...
 advanceDefaultBranchArea() {
   ...
   if (this.model.search != "NAME") {
    if(this.model.search == "TYPE"){
      var headers = new HttpHeaders({ 'branchCode': 'B1', 'journeyId': 'A1', 'userId': 'User1' });
      this.SpinnerService.show();
      this.httpservice.get<any>(AppSetting.API_ENDPOINT + `secure/v1/branches/types`, { headers: headers }).subscribe(
        data => {
        let ob = ErrorConstants.validateException(data);
          if(ob.isSuccess){
 ...
 advanceDefaultBranchAreaById() {
  ...
  this.httpservice
    .get<any>(
     AppSetting.API_ENDPOINT + `secure/v1/branches/${this.model.search}/` + this.model.areaId.branchId,
      { headers: headers }
    )
    .subscribe(data => {
    let ob = ErrorConstants.validateException(data);
...
this.arrdata=[];
 ...
 let apiEndPoint ='';
     ...
     if (this.model.search != "NAME") {
     if(this.model.search == "TYPE"){apiEndPoint = AppSetting.API_ENDPOINT +`secure/v1/branches/types`}
     else if(this.model.search != "PINCODE"){
