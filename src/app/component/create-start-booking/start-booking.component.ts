...
@Component({
  selector: "app-create-start-booking",
  templateUrl: "./start-booking.component.html",
  styleUrls: ["./start-booking.component.css"],
})
export class CreateStartBookingComponent implements OnInit {
  ...
  saveWayBill(type) {
    ...
    console.log('this.bookinginfo.ncv flag', this.bookingInfoObj);
    if (this.bookingInfoObj.ncvFlag !== 1) {
      if (!this.bookingInfoObj.invoiceList) {
        this.appComp.showMessage(`Add Invoice Details !`, 'danger')
        return;
      }
      if (this.bookingInfoObj.invoiceList && this.bookingInfoObj.invoiceList.length === 0) {
        this.appComp.showMessage(`Add Invoice Details !`, 'danger')
        return;
      }
    }
    ...
    this.bookingInfoObj.reBookingFlag = 1;
  ...
  initialEditWaybillDetails() {
    console.log('initialEditWaybillDetails')
        ...
        .subscribe((response) => {
          ...
          if (response) {
            ...
            if (this.refType(Number(this.requestedMop)) === 'PAID') {
              const toPay = this.allLookupList.find(elem => elem.lookupTypeVal.trim() == "BRANCH_MOP" &&
                elem.lookupVal.trim() == 'TO-PAY');
              const prc = this.allLookupList.find(elem => elem.lookupTypeVal.trim() == "CUST_TYPE" &&
                elem.lookupVal.trim() == 'PRC');
              this.bookingInfoObj.branchMOPLookupId = toPay.id;
            ...
            // this.bookingInfoObj.hubDeliveryFlag = 0;
            if (updatedOffering) {
              this.oldOfferingId = this.bookingInfoObj.serviceOfferingId;
            ...
            if (isRetailCheck == 1) {
              ...
              if (Number(this.bookingInfoObj.serviceOfferingId) !== Number(this.oldOfferingId)) {
                console.log('inside new ratecard offering change')
  ...
  initialNewWaybillDetails() {
    console.log('initialNewWaybillDetails')

    try {
      var wayBillId = sessionStorage.getItem("newWaybill");
      console.log('waybillllllll Idddddddd', wayBillId);
      this.$bookingInfo
        .getWaybillDetailByWaybillId(wayBillId, this.headerData)
        .subscribe((response) => {
  ...
  searchSafexId(sfxId) {
    // console.log('inside searchSafexId')
        ...
        this.$bookingInfo
          ...
          .subscribe((Response) => {
            ...
            if (Response) {
              ...
              // console.log('this.selectedcommercial', this.selectedCommercial);
  ...
  getProductCategoriBycommercialCatId(obj) {
    // console.log('getProductCategoriBycommercialCatId')
    try {
      if (this.fieldsetDisabled == false) {
        this.spinner.show();
        if (!obj.prdctCtgyId) {
        ...
        // console.log('############# this.obj.value', obj)
