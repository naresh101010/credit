...
@Component({
  selector: 'app-create-pre-booking',
  templateUrl: './create-pre-booking.component.html',
  styleUrls: ['./create-pre-booking.component.scss']
})
export class CreatePreBookingComponent implements OnInit {
 @ViewChild("step1", null) consigneeInfo: any;
        ...
        if(this.bookingInfoObj.consigneeId){
          this.$bookingInfo.getConsignorConsigneeById(this.bookingInfoObj.consigneeId).subscribe(
            (response) => {
              ...
              let returnVal  = 0 ;
            this.consigneeList.forEach(elem=>{
            if(elem.addrBook.pincodeId == this.destPincode){
              returnVal = 1;
            }
            })  
            if(!this.isShowListOfCnorcee){
  ...
  navigateIt(){
    this.waybillService.preBooking_tab.next(true);
  ...
  validateWaybillNummanually(event) {
    ...
    if (strlength.toString().length > 14 && !this.bookingInfoObj.autoWaybill) {
        ...
        if (Response) {
          if(this.bookingInfoObj.ewaybillAvailableFlag){
            this.bookingInfoObj.invoiceList = Response.invoiceList;
             ...
             if(this.isWaybillNoDisable == true){
...
saveConsignorConsigneeInfo(){
  ...
  saveWayBill(type) {
    ...
    let  isPresent = [] as any;
    if(this.isGeneralRetail){
       isPresent = this.selectedProductCategoryCardList.filter(
        (elem) =>
    ...
    if (this.bookingInfoObj.packageList && this.bookingInfoObj.packageList.length) {
      this.bookingInfoObj.packageList.forEach((elm , index) => {
        let seletedindex : any;
        if(this.isGeneralRetail){
         seletedindex  = this.selectedProductCategoryCardList.findIndex(elem=> elem.localId == elm.localId)
       }
       else {
        seletedindex  = this.selectedProductCategoryCardList.findIndex(elem=> elem.id == elm.productCategoryId)
       }
       if(seletedindex == -1 || seletedindex == undefined){
      ...
      .subscribe((Response) => {
        if (Response) {
          ...
          if (Response.deliveryGatewayBranchId) {
            ...
            this.bookingInfoObj.deliveryGatewayBranchId = '';
          }
          ...
          if (Response.deliveryGatewayBranchName) {
            ...
            this.bookingInfoObj.deliveryGatewayBranchName = '';
          }
          this.IsCrate = false;
          // this.appComp.showMessage(
  ...
  getAllLoad() {
    ...
    this.$bookingInfo.getLoad(this.branchId).subscribe((Response) => {
      ...
      if (sessionStorage.getItem("bookingReqId")) {
        ...
        // this.editWaybillDetails(sessionStorage.getItem("bookingReqId"));
      }
      else {
  ...
  RefreshOnChange(type = null) {
    ...
    if (this.fieldsetDisabled == false) {
      ...
      // if(this.bookingInfoObj.packageList){
      //   this.bookingInfoObj.packageList.forEach(element => {
      //     let obj = this.selectedProductCategoryList.find(elem=> elem.id == element.productCategoryId)
      //   });
      // }
  ...
  getRateCardWithoutBranch(){
    this.$bookingInfo
    .getRatecardByContractWithoutBranch(this.sfxObj.id, this.bookingInfoObj.serviceOfferingId, this.contractType , this.headerData)
    .subscribe((response) => {
      if (response) {
        var TotalRateCard = [];
        var isPrcRateCard = [];
...
var response : any;
if(business_type.lookupVal.trim() != 'ANYWHERE TO ANYWHERE'){
response =   this.$bookingInfo.getRatecardByContractWithoutBranch(this.sfxObj.id, this.bookingInfoObj.serviceOfferingId, this.contractType , this.headerData)
}else{
response =  this.$bookingInfo.gettingRatecardByContract(this.bookingInfoObj.sfxPrcContractcode, this.contractType, this.headerData2);
}
  ...
  searchSafexId(sfxId) {
    ...
    if (this.selectedValue == "SFX" || this.selectedValue == "PRC") {
      ...
      if (strlength > 2 && sfxId) {
        ...
        var response : any;
        if(business_type.lookupVal.trim() != 'ANYWHERE TO ANYWHERE'){
        response = this.$bookingInfo.getSfxCode(sfxId, this.contractType, this.headerData);
        }else{
        response = this.$bookingInfo.getContractById(this.bookingInfoObj.prcCntrId, this.contractType, this.headerData);
        }
        response.subscribe(
          (response) => {
            if (response) {
  ...
  handleChange(evt) {
    ...
    if (evt.value) {
      ...
      this.localInvoiceList = [...this.bookingInfoObj.invoiceList];
      this.bookingInfoObj.invoiceList = [];
      this.bookingInfoObj.ewaybillRequirementFlag = 0;
      this.bookingInfoObj.ncvFlag = 0;
      this.invoiceCost = 0;
    ...
    this.bookingInfoObj.destPincodeId = "";
if(this.localInvoiceList.length){
  this.bookingInfoObj.invoiceList =[...this.localInvoiceList];
}
else{
  this.bookingInfoObj.invoiceList.push({
    invoiceNumber: "",
              ...
              if (this.commercialTypeList.length == 1) {
                ...
                this.saveBookingInfoDetails();
                 if(this.consigneeInfo.valid){
                  ...
                  if(this.bookingInfoObj.waybillId && !this.sampleBody.consigneeId){
                    this.selectedType = 2;
                    this.getExistingConsignee(this.selectedType);
                  }
                }
              }
            }
...
if(this.sampleBody.consigneeId){
          ...
          .subscribe((response) => {
            ...
            if (response) {
              ...
              if (response.prdctCtgy.trim() == "GENERAL") {
                ...
                //   if(this.preBookingRequestObj.productCategory){
                //     let productCategory = this.categoryListWithAlisList.find(elem=> elem.prdctCtgy == this.preBookingRequestObj.productCategory)
    ...
    if (this.fieldsetDisabled == false) {
      ...
      dialog.afterClosed().subscribe((response) => {
        ...
        if (response) {
          ...
          this.addConsignorList.push(response)
    ...
    this.$bookingInfo
      ...
      .subscribe((response) => {
        if (response) {
          ...
          if (this.consignerList != undefined) {
            if (this.consignerList.length > 1) {
              var filterList  = [] as any;
              if(this.bookingInfoObj.consignorId){
                let consignor = this.consignerList.find(elem=> elem.id  == this.bookingInfoObj.consignorId)
                if(consignor){
                   filterList = this.consignerList.filter(elem => elem.name == consignor.name && elem.addrBook.pincodeId == consignor.addrBook.name)
                if(filterList.length){
                  this.consignerList = [...filterList];
                }
                }
              }
              else if(this.preBookingRequestObj.consignorName && this.preBookingRequestObj.consignorPincodeId){
                 filterList = this.consignerList.filter(elem => elem.name == this.preBookingRequestObj.consignorName && elem.addrBook.pincodeId == this.preBookingRequestObj.consignorPincodeId)
                if(filterList.length){
                  this.consignerList = [...filterList];
                }
              }
      ...
      if (response) {
        ...
        if(this.consigneeList.length > 1){
          let filterList = [] as any;
           if(this.bookingInfoObj.consigneeId){
                let consignee = this.consigneeList.find(elem=> elem.id  == this.bookingInfoObj.consigneeId)
                if(consignee){
                   filterList = this.consigneeList.filter(elem => elem.name == consignee.name && elem.addrBook.pincodeId == consignee.addrBook.pincodeId)
                if(filterList.length){
                  this.consigneeList = [...filterList];
                }
                }
              }
              else if(this.preBookingRequestObj.consigneeName && this.preBookingRequestObj.consigneePincodeId){
                 filterList = this.consigneeList.filter(elem => elem.name == this.preBookingRequestObj.consigneeName && elem.addrBook.pincodeId == this.preBookingRequestObj.consigneePincodeId)
                if(filterList.length){
                  this.consigneeList = [...filterList];
                }
              }
        ...
        if (response) {
          ...
          if (this.destPincode) {
            ...
            if (this.consigneeList.length > 1) {
              let filterList = [] as any;
              if(this.bookingInfoObj.consigneeId){
                   let consignee = this.consigneeList.find(elem=> elem.id  == this.bookingInfoObj.consignorId)
                   if(consignee){
                      filterList = this.consigneeList.filter(elem => elem.name == consignee.name && elem.addrBook.pincodeId == consignee.addrBook.name)
                   if(filterList.length){
                     this.consigneeList = [...filterList];
                   }
                   }
        ...
        if (response) {
          ...
          this.$bookingInfo.getAllConsignorExistingWithOutRateCard(this.headerData).subscribe((res) => {
            if (res) {
              ...
              this.consignerList.sort((a, b) => a.name.localeCompare(b.name));
              if (this.addConsignorList.length) {
                this.addConsignorList.forEach(elem => {
                  this.consignerList.splice(0, 0, elem);
                })
              }
              this.localConsignorList = [...this.consignerList];
  ...
  openEpayment() {
    let data = {
      ...
      Tamount: this.grandTotalCharges
    }
    var dialog = this.dialog.open(EpaymentRequestComponent, {
      width: "85rem",
      panelClass: "bookingDialog",
      data:data
    });
    ...
    dialog.afterClosed().subscribe((res: any) => {
      if (res) {
        console.log(res);
        this.router.navigate(['/bookings-web-booking/payment-rcpt', { 'receiptno': res }]);
      }
    });
  }
  ...
  getFilterProductCategory(product, i) {
    ...
    if (this.fieldsetDisabled == false) {
      ...
      if (this.isPeaceType == true) {
        ...
        if (this.bookingInfoObj.packageList) {
          let presentVal = 0
          this.bookingInfoObj.packageList.forEach((elem , index) => {
            if(!elem.pkgBreadth){
              this.bookingInfoObj.packageList.splice(index,1);
            }
            if(this.isGeneralRetail){
  ...
  deleteProductCategory(product, index) {
    ...
    if (this.fieldsetDisabled == false) {
      if (this.selectedProductCategoryList.length) {
        if(this.isGeneralRetail){
         if(this.retailGeneralproductcategory){
...
getWaybillByReqId(reqId){
  ...
  this.$bookingInfo.getWaybillByRequestId(obj , this.headerData).subscribe((response)=> {
    if(response.responseData.length){
      this.bookingInfoObj.waybillId = response.responseData[0].waybillId;
     this.editWaybillDetails();
    }
    else{
      this.bookingInfoObj = {...this.preBookingRequestObj};
      this.sampleBody = {...this.preBookingRequestObj};
      this.bookingInfoObj.actualWeight = this.preBookingRequestObj.totalWeight;
      this.bookingInfoObj.branchMOPLookupId = this.preBookingRequestObj.mopId;
      this.bookingInfoObj.businessTypeLookupId = this.preBookingRequestObj.businessTypeLkpId;
      this.bookingInfoObj.consignorId = this.preBookingRequestObj.consignorId;
      this.bookingInfoObj.consigneeId = this.preBookingRequestObj.consigneeId;
      this.bookingInfoObj.custTypeLookupId = this.preBookingRequestObj.customerTypeLkpId;
      this.bookingInfoObj.pickupDate = this.preBookingRequestObj.pickUpDate;
      this.bookingInfoObj.prcCntrId = this.preBookingRequestObj.prcContractId;
      this.bookingInfoObj.serviceOfferingId = this.preBookingRequestObj.serviceOfferingId;
      this.bookingInfoObj.sfxPrcContractcode = this.preBookingRequestObj.prcContractCode;
      this.bookingInfoObj.totalPackageCount = this.preBookingRequestObj.totalPackageCount;
      this.bookingInfoObj.waybillNumber = this.preBookingRequestObj.waybillNo;
      this.bookingInfoObj.pickupTime ='';
  this.bookingInfoObj.lkpBookingRequestTypeId = this.preBookingRequestObj.bookingRequestTypeId;
      this.bookingInfoObj.rateCardCode = this.preBookingRequestObj.rateCardCode;
      this.bookingInfoObj.bookingReqId = this.preBookingRequestObj.preBookingRequestId;
      this.bookingInfoObj.destPincodeLookupId = this.preBookingRequestObj.consigneePincodeId;
      this.destPincode = this.preBookingRequestObj.consigneePincodeId;
      if(this.preBookingRequestObj.packageList){
      this.bookingInfoObj.packageList = this.preBookingRequestObj.packageList;
      }
      else{
        this.bookingInfoObj.packageList = [] ;
      }  if(this.bookingInfoObj.waybillNumber){
        this.validateWaybillNummanually(this.bookingInfoObj.waybillNumber);
      }

      this.bookingInfoObj.invoiceList = [];
      if (!this.preBookingRequestObj.ewaybillAvailableFlag) {
  ...
  getPreBookingDetails(wayBillId) {
    try {
       
      // var wayBillId = sessionStorage.getItem("waybillId");
      // console.log('waybillllllll Idddddddd', wayBillId);
      this.spinner.show();
      this.openBookingService.getBookingRequestDetailsByPre(wayBillId)
        .subscribe((response : any) => {
          console.log(response);
          this.preBookingRequestObj  = response.data.responseData.prebookingDetailList[0];

          if(this.preBookingRequestObj.bookingRequestTypeId){
          this.getWaybillByReqId(wayBillId);
          }
        });
    } catch (error) {
      this.spinner.hide();
  ...
  getPreBookingResponse(){
      ...
      if(this.bookingInfoObj.pickUpFromTime){
        ...
        let splitLen = time.split(":");
        if(splitLen[2] == "PM" || splitLen[2] == "AM"){
      ...
      if(this.bookingInfoObj.chargeWeight){
       ...
       if(!this.bookingInfoObj.ncvFlag){
        this.bookingInfoObj.ncvFlag = 0;
       }
      ...
      business_type = this.allLookupList.find(elem => elem.lookupTypeVal.trim() == "BIZ_TYPE" &&
        ...
        if(business_type.lookupVal.trim() == 'ANYWHERE TO ANYWHERE' || business_type.lookupVal.trim() == 'INBOUND'){
      ...
      this.allLookupList.forEach((elem) => {
        if (elem.lookupVal.trim() == "CREDIT" &&
          this.bookingInfoObj.custTypeLookupId == elem.id && (business_type.lookupVal.trim() == 'OUTBOUND' || business_type.lookupVal.trim() == 'INBOUND' || business_type.lookupVal.trim() == 'ANYWHERE TO ANYWHERE')) {
          this.isCreditCheck = true;
          this.contractOption = 1;
          this.selectedValue = 'SFX';
          this.MopName = 'CREDIT';
        ...
        if (elem.lookupVal.trim() == "CREDIT+RETAIL" &&
        this.bookingInfoObj.custTypeLookupId == elem.id) {
          isRetailCheck = 1;
          this.isRetail =false;
          this.getServiceOffering();
    ...
    //   }
      ...
      // }
      if(this.bookingInfoObj.consigneePincodeId){
        this.gettingPincode(this.bookingInfoObj.consigneePincodeId);
      }
