...
export class HomeComponent implements OnInit {
  ...
  columnsToDisplay: string[] = ['subEntityName', "moduleName", "entityName", 'create', 'update', 'read', 'symbol', 'delete'];
...
// Web permission code Start ----
  ...
  readMap(data, $event) {
    ...
    for (var i = this.finalObjectAttributePermissions.length - 1; i >= 0; --i) {
        if (this.finalObjectAttributePermissions[i].objectId == data.objectId &&
         this.finalObjectAttributePermissions[i].permissionId == 3
    ...
    if(data.attributeExclutionList != undefined){
      ...
      for (var i = this.finalObjectAttributePermissions.length - 1; i >= 0; --i) {
        if (this.finalObjectAttributePermissions[i].objectId == data.objectId && this.finalObjectAttributePermissions[i].permissionId == 3
        && this.finalObjectAttributePermissions[i].channelId == 33) {
    ...
    removeObject(value, j) {
          ...
          for (var i = this.webobject.length - 1; i >= 0; --i) {
           ...
           let permissionIds = [1,2,3];
              ...
              let removeObjectIndex = this.finalObjectAttributePermissions.findIndex(object=>(object.objectId === value.objectId
                      && object.channelId === value.channelId && object.permissionId ===permissionId))
                if(removeObjectIndex >=0){
                this.finalObjectAttributePermissions.splice(removeObjectIndex,1);
                }
              }
                  this.webobject =  [...this.webobject];
                  this.mobileobject = [...this.mobileobject];
    ...
    if ($event.checked == true) {
      ...
      if(data.attributeExclutionList != undefined){
       ...
       data['isCreateUpdateAttributePermission'] = true;
        ...
        for (var i = this.finalObjectAttributePermissions.length - 1; i >= 0; --i) {
          if (this.finalObjectAttributePermissions[i].objectId == data.objectId
          && this.finalObjectAttributePermissions[i].permissionId == 1
         && this.finalObjectAttributePermissions[i].channelId ===33) {
  ...
  updateMap(data, $event) {
    ...
    if ($event.checked == true) {
      ...
      for (var i = this.finalObjectAttributePermissions.length - 1; i >= 0; --i) {
              if (this.finalObjectAttributePermissions[i].objectId == data.objectId && this.finalObjectAttributePermissions[i].permissionId == 2
              && this.finalObjectAttributePermissions[i].channelId == 33) {
                this.finalObjectAttributePermissions.splice(i, 1);
                break
              }
  ...
  if ($event.checked == true) {
    ...
    this.MupdateMap(data, $event);
     data['isCreateUpdateAttributePermission'] = true;
      ...
      for (var i = this.finalObjectAttributePermissions.length - 1; i >= 0; --i) {
        if (this.finalObjectAttributePermissions[i].objectId == data.objectId && this.finalObjectAttributePermissions[i].permissionId == 1
        && this.finalObjectAttributePermissions[i].channelId ===34) {
          this.finalObjectAttributePermissions.splice(i, 1);
          break;
      }
...
export class DialogContentExampleDialog implements OnInit {
      ...
      .subscribe((users: any) => {
        ...
        let assignedMobilePermissions =  _.find(this.data.assignedMobileObject, (obj) => (obj.objectId == data.objectId && obj.channelId == data.channelId));
          ...
          if (data.channelId == 33) {
           ...
           if(assignedObjects.objectChannelMapStatus != undefined && assignedObjects.objectChannelMapStatus == 1){
  ...
  if(data.channelId==34){
    ...
    if ($event.checked == true) {
      ...
      for (var i = this.objectList.length - 1; i >= 0; --i) {
        if (this.objectList[i].subEntityName == data.subEntityName &&
        this.objectList[i].objectId == data.objectId) {
          this.objectList.splice(i, 1);
        }
